...
function getStatsData(time, format, species, cb) {
  util.cacheGetOrCalculateAndSet(`${time}:${format}:${species}:stats`, cb => {
    getRawPokemonData(time, format, species, (err, data) => {
      calculateStatsData(species, data, cb)
    })
  }, 3600, cb)
}

function calculateStatsData(species, pokemonData, cb) {
  let hp = {};
  let atk = {};
  let def = {};
  let satk = {};
  ...
  for (var [spread, amount] of Object.entries(pokemonData.Spreads)) {
    total += amount;
    ...
    stats = {
      ...
      satk: calculateStat(level, baseStats.spa, evs[3], natureMultipliers[nature][3], false),
      sdef: calculateStat(level, baseStats.spd, evs[4], natureMultipliers[nature][4], false),
      spd: calculateStat(level, baseStats.spe, evs[5], natureMultipliers[nature][5], true),
    }
...
function calculateStatsDataSync(species, pokemonData) {
  let hp = {};
  let atk = {};
  let def = {};
  let satk = {};
